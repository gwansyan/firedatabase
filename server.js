...
app.get('/sw.js',(_,res)=>res.type('application/javascript').send(`
...
app.get('/edu',(_,res)=>res.type('html').send(String.raw`<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RT7 EDU V2A Push Debug</title><style>
body{font-family:Arial,'Noto Sans TC',sans-serif;background:#eef4f6;margin:0;color:#10232e}.wrap{max-width:1040px;margin:20px auto;padding:16px}.card{background:#fff;border-radius:14px;padding:18px;margin:14px 0;box-shadow:0 2px 8px #0001}input,select,button{font-size:16px;padding:10px;border-radius:8px;border:1px solid #ccd6dc;margin:4px}button{background:#0b9b5a;color:#fff;border:0}.blue{background:#0b78d0}.red{background:#c0392b}.gray{background:#64748b}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:8px}table{width:100%;border-collapse:collapse}th,td{padding:8px;border-bottom:1px solid #e5edf1;text-align:left;vertical-align:top}pre{background:#f5f7f8;padding:10px;border-radius:8px;overflow:auto;white-space:pre-wrap}.ok{color:#079b50;font-weight:bold}.bad{color:#d33;font-weight:bold}.hint{color:#607080;font-size:14px;line-height:1.6}.pill{display:inline-block;padding:3px 8px;border-radius:999px;background:#e9f7ef;color:#0b7a43;font-weight:bold}.badp{background:#fee2e2;color:#b91c1c}.warnp{background:#fef3c7;color:#92400e}</style></head><body><div class="wrap"><h1>RT7 EDU TEST V2A：Push Send Result Debug</h1><p>顯示 sent / total / failures，並提供重新訂閱推播。</p><div id="app">載入中...</div></div><script>
...
h+='<div class="card"><h2>2. 推播訂閱與診斷</h2><p class="hint">先在手機按「重新訂閱推播」。若手機沒跳通知，請看最新 push log 的 sent/total/failures。</p><button class="blue" onclick="subscribePush()">啟用推播</button><button class="blue" onclick="resubscribePush()">重新訂閱推播</button><button onclick="testPush()">測試推播</button><button class="red" onclick="clearPush()">清除伺服器訂閱紀錄</button><button class="gray" onclick="checkClientPush()">檢查本機瀏覽器訂閱</button><p>訂閱數：<span class="pill '+(s.push.count>0?'':'warnp')+'">'+esc(s.push.count)+'</span>　最新推播：'+pill(latest)+'　最後時間：'+esc((latest&&latest.time)||'-')+'</p><details open><summary>最新 push log 詳細內容</summary><pre>'+esc(JSON.stringify(latest||{},null,2))+'</pre></details></div>';
...
h+='<div class="card"><h2>4. 門鈴 / 開門 / 推播測試</h2><div class="grid"><input id="e_uid" placeholder="Master UID" value="'+esc((ms[0]&&ms[0].master_uid)||'')+'"><input id="e_msg" value="有人按門鈴"></div><button onclick="doorbell()">送出門鈴事件 + 推播</button><button onclick="openDoor()">送出開門命令</button><button onclick="pollCmd()">ESP32 輪詢命令</button></div>';
h+='<div class="card"><h2>5. 事件紀錄</h2><table><tr><th>時間</th><th>UID</th><th>事件</th><th>訊息</th><th>Push sent</th><th>Push ok</th></tr>'; (s.events||[]).slice(-10).reverse().forEach(e=>h+='<tr><td>'+esc(e.time)+'</td><td>'+esc(e.master_uid)+'</td><td>'+esc(e.event)+'</td><td>'+esc(e.message)+'</td><td>'+esc(e.push_sent==null?'-':e.push_sent)+'</td><td>'+esc(e.push_ok==null?'-':e.push_ok)+'</td></tr>');h+='</table></div><div class="card"><h2>6. 回應</h2><pre id="out">READY</pre></div>';document.getElementById('app').innerHTML=h;}
...
async function subscribePush(){if(!('serviceWorker'in navigator)||!('PushManager'in window)){out({ok:false,error:'browser_not_supported'});return;}const kr=await api('/api/push/public-key');if(!kr.publicKey){out({ok:false,error:'no_vapid_key',detail:kr});return;}const reg=await navigator.serviceWorker.register('/sw.js');const perm=await Notification.requestPermission();if(perm!=='granted'){out({ok:false,error:'notification_permission_'+perm});return;}let sub=await reg.pushManager.getSubscription();if(!sub)sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:b64ToUint8Array(kr.publicKey)});out(await post('/api/push/subscribe',{subscription:sub}));await load();}
async function resubscribePush(){const reg=await navigator.serviceWorker.register('/sw.js');const old=await reg.pushManager.getSubscription();if(old)try{await old.unsubscribe();}catch(e){}await clearPush(false);await subscribePush();}
async function checkClientPush(){const regs=await navigator.serviceWorker.getRegistrations();let arr=[];for(const r of regs){const sub=await r.pushManager.getSubscription();arr.push({scope:r.scope,subscribed:!!sub,endpoint:sub&&sub.endpoint});}out({ok:true,permission:Notification.permission,registrations:arr});}
async function testPush(){out(await post('/api/push/test',{title:'RT7 EDU 測試推播',body:'推播功能正常'}));await load();}
async function clearPush(show=true){const r=await post('/api/push/clear',{});if(show)out(r);await load();return r;}
...
load();setInterval(async()=>{try{const s=await api('/edu/state'),a=document.activeElement;if(!a||!['INPUT','SELECT','TEXTAREA'].includes(a.tagName))render(s);}catch(e){}},5000);
</script></body></html>`));

app.get('/edu/state',(_,res)=>{const masters=readJson('master_registry.json',{});Object.keys(masters).forEach(uid=>masters[uid].status=onlineStatus(masters[uid].last_heartbeat));const logs=readJson('push_log.json',[]).slice(-10).reverse();res.json({ok:true,masters,communities:readJson('communities.json',[]),events:readJson('events.json',[]),commands:readJson('commands.json',{}),push:{count:readJson('push_subscriptions.json',[]).length,logs,webpush:!!webpush,setup:setupWebPush()}});});
app.post('/edu/master/heartbeat',(req,res)=>{let{master_uid,ip,mac}=req.body||{};if(!master_uid)master_uid=uidFromMac(mac);if(!master_uid)return res.status(400).json({ok:false,error:'missing master_uid_or_valid_mac'});const masters=readJson('master_registry.json',{}),existing=masters[master_uid]||{},source=detectSource(mac);if(existing.source==='ESP32'&&source==='SIM'){existing.last_sim_heartbeat=nowIso();masters[master_uid]=existing;}else masters[master_uid]={master_uid,ip:ip||req.ip,mac:mac||existing.mac||'',last_heartbeat:nowIso(),status:'ONLINE',source,uid_rule:'MAC_REVERSE_PAIRS'};writeJson('master_registry.json',masters);res.json({ok:true,master:masters[master_uid]});});
...
app.listen(PORT,()=>console.log('[RT7_EDU_TEST_V2A_PUSH_DEBUG] http://localhost:'+PORT+'/edu'));
