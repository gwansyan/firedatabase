...
function rt7V14GindLatestSnapshotFile_() {
  const candidates = [
...
app.get('/edu/ai-voice-assistant-v14g.js', (_req, res) => {
...
(function(){
  ...
  function speak(t){ try{ if(!window.speechSynthesis){ addLog('[TTS] not supported'); return; } stopRecognition(); window.speechSynthesis.cancel(); var u=new SpeechSynthesisUtterance(String(t||'')); u.lang='zh-TW'; u.rate=1; u.pitch=1; u.onstart=function(){stopRecognition();}; window.speechSynthesis.speak(u); }catch(e){ addLog('[TTS_ERROR] '+e); } }
  function reloadSnap(){ var img=el('snap'); var no=el('noSnap'); if(no) no.style.display='none'; if(img){ img.style.display='block'; img.src='/edu/face/latest.jpg?_='+Date.now(); } }
  async function unlockMicPermission(){
    if(!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){ status('此手機瀏覽器不支援 getUserMedia；直接嘗試語音辨識'); addLog('[MIC_UNLOCK] no getUserMedia'); return true; }
    status('正在啟用手機 MIC 權限...'); addLog('[MIC_UNLOCK] request getUserMedia');
    try{
      var stream = await navigator.mediaDevices.getUserMedia({audio:true, video:false});
      var tracks = stream.getAudioTracks ? stream.getAudioTracks() : [];
      addLog('[MIC_UNLOCK] granted tracks='+tracks.length+' label='+(tracks[0]&&tracks[0].label||''));
      try{ if(stream.getTracks) stream.getTracks().forEach(function(t){ try{t.stop();}catch(e){} }); }catch(e){}
      setMicUi(true);
      status('MIC 權限已開啟，已釋放 MIC，準備啟動 Chrome 語音辨識');
      await new Promise(function(resolve){ setTimeout(resolve, 700); });
      return true;
    }catch(e){ setMicUi(false); status('手機 MIC 啟用失敗：'+(e && (e.name||e.message) || e)+'。請到 Chrome 網站設定允許麥克風。'); addLog('[MIC_UNLOCK_ERROR] '+(e && (e.stack||e.message)||e)); return false; }
  }
  function askText(q){ q=String(q || (el('question')&&el('question').value) || '').trim() || '現在看到什麼？'; if(el('question')) el('question').value=q; if(busy){status('AI 正在處理，請稍候');return;} busy=true; stopSpeak(); stopRecognition(); setText('answer','AI 分析中...'); status('送出問題：'+q); fetch('/api/v14/voice/camera-qa?_='+Date.now(),{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify({question:q})}).then(function(r){return r.text().then(function(t){return{status:r.status,text:t};});}).then(function(o){var j;try{j=JSON.parse(o.text);}catch(e){j={ok:false,error:'NON_JSON',raw:o.text,http_status:o.status};} log(j); if(j&&j.ok){setText('answer',j.answer||'(沒有回答)');status('AI 回答完成');reloadSnap();speak(j.answer||'完成');}else{setText('answer','錯誤：'+((j&&(j.error||j.raw))||'unknown'));status('AI 回答失敗');}}).catch(function(e){log('[FETCH_ERROR] '+(e&&(e.stack||e.message)||e));setText('answer','送出失敗：'+e);status('送出失敗');}).then(function(){busy=false;}); }
  async function startMic(){
    log('MIC_CLICK V14G\nSpeechRecognition='+(!!SR)+'\ngetUserMedia='+(!!(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia))+'\nprotocol='+location.protocol+'\nhost='+location.host);
    stopSpeak(); stopRecognition();
    var ok=await unlockMicPermission(); if(!ok)return;
    if(!SR){status('此瀏覽器不支援 SpeechRecognition。請改用手機 Chrome，或先用文字詢問。');return;}
    try{
      rec=new SR(); rec.lang='zh-TW'; rec.interimResults=true; rec.continuous=false; rec.maxAlternatives=1;
      var gotAny=false;
      rec.onstart=function(){addLog('[SR_ONSTART]');status('語音單次聆聽中，請說：現在看到什麼？');hearTimer=setTimeout(function(){if(!gotAny){try{rec.stop();}catch(e){} status('沒有聽到語音，請再按一次麥克風');}},9000);};
      rec.onaudiostart=function(){addLog('[SR_AUDIO_START]');status('麥克風已交給語音辨識，請說話');};
      rec.onsoundstart=function(){addLog('[SR_SOUND_START]');};
      rec.onspeechstart=function(){gotAny=true;addLog('[SR_SPEECH_START]');status('已偵測到語音，正在辨識');};
      rec.onspeechend=function(){addLog('[SR_SPEECH_END]');try{rec.stop();}catch(e){}};
      rec.onnomatch=function(){addLog('[SR_NOMATCH]');status('沒有辨識到文字，請再按一次麥克風並說：現在看到什麼？');};
      rec.onerror=function(e){var er=(e&&e.error)||'unknown';addLog('[SR_ERROR] error='+er+' raw='+JSON.stringify(e||{})); if(er==='aborted')status('語音辨識被中止。V14G 已釋放 MIC，請再按一次麥克風；若仍失敗請關閉重開 Chrome 分頁。'); else if(er==='no-speech')status('沒有聽到語音，請再按一次麥克風。'); else if(er==='not-allowed'||er==='service-not-allowed')status('Chrome 擋住語音辨識/麥克風，請允許網站麥克風權限。'); else status('語音辨識錯誤：'+er+'，請再按一次麥克風。');};
      rec.onend=function(){if(hearTimer){clearTimeout(hearTimer);hearTimer=null;}addLog('[SR_END]');};
      rec.onresult=function(ev){if(hearTimer){clearTimeout(hearTimer);hearTimer=null;} gotAny=true; var finalText='',interimText=''; for(var i=ev.resultIndex||0;i<ev.results.length;i++){var txt='';try{txt=ev.results[i][0].transcript||'';}catch(e){} if(ev.results[i].isFinal)finalText+=txt;else interimText+=txt;} if(finalText){status('辨識完成：'+finalText);askText(finalText);}else if(interimText){status('正在聽：'+interimText);addLog('[SR_INTERIM] '+interimText);}};
      setTimeout(function(){try{rec.start();addLog('[SR_START_CALLED_AFTER_MIC_RELEASE]');}catch(e){status('SpeechRecognition 啟動失敗：'+(e.message||e));addLog('[SR_START_ERROR] '+(e.stack||e.message||e));}},180);
    }catch(e){status('麥克風流程啟動失敗：'+(e&&(e.message||e)||e));addLog('[MIC_FLOW_ERROR] '+(e&&(e.stack||e.message)||e));}
  }
  function bind(){window.rt7V14AskText=askText;window.rt7V14StartMic=startMic;window.rt7V14Stop=function(){stopSpeak();stopRecognition();setMicUi(false);status('已停止 MIC / 播放');};window.rt7V14ReloadSnap=reloadSnap;var b;b=el('btnText');if(b)b.onclick=function(){askText();};b=el('btnMic');if(b)b.onclick=function(){startMic();};b=el('btnStop');if(b)b.onclick=function(){window.rt7V14Stop();};b=el('btnSnap');if(b)b.onclick=function(){reloadSnap();};log('JS_READY V14G\nSpeechRecognition='+(!!SR)+'\ngetUserMedia='+(!!(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia))+'\nprotocol='+location.protocol+'\nhost='+location.host);status('JS_READY V14G，文字詢問可測；麥克風會先開啟權限、釋放 MIC，再啟動語音辨識。');}
...
app.get('/edu/ai-voice-assistant-v14g.js', (_req, res) => {
  res.type('application/javascript').send("\n(function(){\n  var SR = window.SpeechRecognition || window.webkitSpeechRecognition || null;\n  var rec = null, busy = false, hearTimer = null, micStream = null, micReady = false;\n  function el(id){ return document.getElementById(id); }\n  function setText(id, text){ var e=el(id); if(e) e.textContent = text; }\n  function log(x){ var d=el('debug'); if(d) d.textContent = (typeof x==='string') ? x : JSON.stringify(x,null,2); }\n  function addLog(x){ var d=el('debug'); if(d) d.textContent = String(d.textContent||'') + '\\n' + x; }\n  function status(x){ setText('micStatus', x); addLog('[STATUS] '+x); }\n  function setMicUi(on){ var b=el('btnMic'); if(b){ b.style.outline = on ? '6px solid rgba(34,197,94,.25)' : ''; b.title = on ? 'MIC \u5df2\u555f\u7528\uff0c\u6309\u4e0b\u958b\u59cb\u8046\u807d' : '\u6309\u4e0b\u555f\u7528 MIC \u4e26\u958b\u59cb\u8046\u807d'; } }\n  function stopSpeak(){ try{ if(window.speechSynthesis) window.speechSynthesis.cancel(); }catch(e){} }\n  function stopRecognition(){ try{ if(rec) rec.stop(); }catch(e){} if(hearTimer){ clearTimeout(hearTimer); hearTimer=null; } }\n  function closeMicStream(){ try{ if(micStream){ micStream.getTracks().forEach(function(t){ try{t.stop();}catch(e){} }); } }catch(e){} micStream=null; micReady=false; setMicUi(false); }\n  function speak(t){ try{ if(!window.speechSynthesis){ addLog('[TTS] not supported'); return; } stopRecognition(); window.speechSynthesis.cancel(); var u = new SpeechSynthesisUtterance(String(t||'')); u.lang = 'zh-TW'; u.rate = 1; u.pitch = 1; u.onstart = function(){ stopRecognition(); }; window.speechSynthesis.speak(u); }catch(e){ addLog('[TTS_ERROR] '+e); } }\n  function reloadSnap(){ var img=el('snap'); var no=el('noSnap'); if(no) no.style.display='none'; if(img){ img.style.display='block'; img.src='/edu/face/latest.jpg?_='+Date.now(); } }\n  async function ensureMicPermission(){\n    if(micReady && micStream) return true;\n    if(!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){ status('\u6b64\u624b\u6a5f\u700f\u89bd\u5668\u4e0d\u652f\u63f4 getUserMedia \u9ea5\u514b\u98a8\u6b0a\u9650\uff1b\u8acb\u6539\u7528\u624b\u6a5f Chrome\u3002'); return false; }\n    status('\u6b63\u5728\u555f\u7528\u624b\u6a5f\u9ea5\u514b\u98a8\uff0c\u8acb\u5141\u8a31\u6b0a\u9650...'); addLog('[MIC_PERMISSION] request getUserMedia audio');\n    try{ micStream = await navigator.mediaDevices.getUserMedia({audio:true, video:false}); micReady = true; setMicUi(true); var tracks = micStream.getAudioTracks ? micStream.getAudioTracks() : []; status('\u624b\u6a5f\u9ea5\u514b\u98a8\u5df2\u555f\u7528\uff0c\u8acb\u8aaa\uff1a\u73fe\u5728\u770b\u5230\u4ec0\u9ebc\uff1f'); addLog('[MIC_PERMISSION] granted tracks='+tracks.length+' label='+(tracks[0]&&tracks[0].label||'')); return true; }\n    catch(e){ micReady = false; setMicUi(false); status('\u624b\u6a5f\u9ea5\u514b\u98a8\u555f\u7528\u5931\u6557\uff1a'+(e && (e.name||e.message) || e)+'\u3002\u8acb\u5230 Chrome \u7db2\u7ad9\u8a2d\u5b9a\u5141\u8a31\u9ea5\u514b\u98a8\u3002'); addLog('[MIC_PERMISSION_ERROR] '+(e && (e.stack||e.message)||e)); return false; }\n  }\n  function askText(q){ q = String(q || (el('question') && el('question').value) || '').trim() || '\u73fe\u5728\u770b\u5230\u4ec0\u9ebc\uff1f'; if(el('question')) el('question').value = q; if(busy){ status('AI \u6b63\u5728\u8655\u7406\uff0c\u8acb\u7a0d\u5019'); return; } busy = true; stopSpeak(); stopRecognition(); setText('answer','AI \u5206\u6790\u4e2d...'); status('\u9001\u51fa\u554f\u984c\uff1a'+q); fetch('/api/v14/voice/camera-qa?_='+Date.now(), { method:'POST', cache:'no-store', headers:{'Content-Type':'application/json'}, body:JSON.stringify({question:q}) }).then(function(r){ return r.text().then(function(t){ return {status:r.status, text:t}; }); }).then(function(o){ var j; try{ j=JSON.parse(o.text); }catch(e){ j={ok:false,error:'NON_JSON',raw:o.text,http_status:o.status}; } log(j); if(j && j.ok){ setText('answer', j.answer || '(\u6c92\u6709\u56de\u7b54)'); status('AI \u56de\u7b54\u5b8c\u6210'); reloadSnap(); speak(j.answer || '\u5b8c\u6210'); } else { setText('answer','\u932f\u8aa4\uff1a'+((j && (j.error||j.raw)) || 'unknown')); status('AI \u56de\u7b54\u5931\u6557'); } }).catch(function(e){ log('[FETCH_ERROR] '+(e && (e.stack||e.message) || e)); setText('answer','\u9001\u51fa\u5931\u6557\uff1a'+e); status('\u9001\u51fa\u5931\u6557'); }).then(function(){ busy=false; }); }\n  async function startMic(){ log('MIC_CLICK V14G\\nSpeechRecognition='+(!!SR)+'\\ngetUserMedia='+(!!(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia))+'\\nprotocol='+location.protocol+'\\nhost='+location.host); stopSpeak(); stopRecognition(); var ok = await ensureMicPermission(); if(!ok) return; if(!SR){ status('\u6b64\u700f\u89bd\u5668\u4e0d\u652f\u63f4 SpeechRecognition\u3002MIC \u5df2\u555f\u7528\uff0c\u4f46\u7121\u6cd5\u8a9e\u97f3\u8f49\u6587\u5b57\uff1b\u8acb\u7528\u624b\u6a5f Chrome \u6216\u5148\u7528\u6587\u5b57\u8a62\u554f\u3002'); return; } try{ rec = new SR(); rec.lang = 'zh-TW'; rec.interimResults = true; rec.continuous = false; rec.maxAlternatives = 1; rec.onstart = function(){ status('\u8a9e\u97f3\u55ae\u6b21\u8046\u807d\u4e2d\uff0c\u8acb\u8aaa\uff1a\u73fe\u5728\u770b\u5230\u4ec0\u9ebc\uff1f'); hearTimer=setTimeout(function(){ try{rec.stop();}catch(e){} status('\u6c92\u6709\u807d\u5230\u8a9e\u97f3\uff0c\u8acb\u518d\u6309\u4e00\u6b21\u9ea5\u514b\u98a8'); }, 10000); }; rec.onaudiostart = function(){ status('\u9ea5\u514b\u98a8\u5df2\u958b\u555f\uff0c\u8acb\u8aaa\u8a71'); }; rec.onspeechstart = function(){ status('\u5df2\u5075\u6e2c\u5230\u8a9e\u97f3\uff0c\u6b63\u5728\u8fa8\u8b58'); }; rec.onspeechend = function(){ try{ rec.stop(); }catch(e){} }; rec.onerror = function(e){ var er=(e&&e.error)||'unknown'; status('\u8a9e\u97f3\u8fa8\u8b58\u932f\u8aa4\uff1a'+er+'\u3002MIC \u6b0a\u9650\u5df2\u958b\uff0c\u8acb\u518d\u6309\u4e00\u6b21\u9ea5\u514b\u98a8\u3002'); addLog('[SR_ERROR] '+JSON.stringify(e||{})); }; rec.onend = function(){ if(hearTimer){ clearTimeout(hearTimer); hearTimer=null; } addLog('[SR_END]'); }; rec.onresult = function(ev){ if(hearTimer){ clearTimeout(hearTimer); hearTimer=null; } var finalText='', interimText=''; for(var i=ev.resultIndex||0;i<ev.results.length;i++){ var txt=''; try{ txt=ev.results[i][0].transcript || ''; }catch(e){} if(ev.results[i].isFinal) finalText += txt; else interimText += txt; } if(finalText){ status('\u8fa8\u8b58\u5b8c\u6210\uff1a'+finalText); askText(finalText); } else if(interimText){ status('\u6b63\u5728\u807d\uff1a'+interimText); } }; try{ rec.start(); addLog('[SR_START_CALLED]'); } catch(e){ status('SpeechRecognition \u555f\u52d5\u5931\u6557\uff1a'+(e.message||e)); addLog('[SR_START_ERROR] '+(e.stack||e.message||e)); } } catch(e){ status('\u9ea5\u514b\u98a8\u6d41\u7a0b\u555f\u52d5\u5931\u6557\uff1a'+(e && (e.message||e) || e)); addLog('[MIC_FLOW_ERROR] '+(e && (e.stack||e.message)||e)); } }\n  function bind(){ window.rt7V14AskText = askText; window.rt7V14StartMic = startMic; window.rt7V14EnableMic = ensureMicPermission; window.rt7V14Stop = function(){ stopSpeak(); stopRecognition(); closeMicStream(); status('\u5df2\u505c\u6b62 MIC / \u64ad\u653e'); }; window.rt7V14ReloadSnap = reloadSnap; var b; b=el('btnText'); if(b) b.onclick=function(){ askText(); }; b=el('btnMic'); if(b) b.onclick=function(){ startMic(); }; b=el('btnStop'); if(b) b.onclick=function(){ window.rt7V14Stop(); }; b=el('btnSnap'); if(b) b.onclick=function(){ reloadSnap(); }; log('JS_READY V14G\\nSpeechRecognition='+(!!SR)+'\\ngetUserMedia='+(!!(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia))+'\\nprotocol='+location.protocol+'\\nhost='+location.host); status('JS_READY V14G\uff0c\u8acb\u5148\u6309\u9ea5\u514b\u98a8\uff0c\u5141\u8a31 MIC \u5f8c\u8aaa\u8a71\uff1b\u4e5f\u53ef\u6309\u6587\u5b57\u8a62\u554f\u6e2c\u8a66\u3002'); }\n  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', bind); else bind();\n})();\n");
});
...
</style></head><body><div class="wrap"><h1>第14堂 AI語音助理 V14G</h1><p class="hint">維持原始 RT7：手機 MIC → Chrome webkitSpeechRecognition → 文字 → Railway Snapshot → OpenAI Vision → 手機 Speaker。</p><div class="top"><a href="/edu/community/register">社區註冊</a><a href="/edu/two-step-liveness">V12B 二步活體</a><a href="/edu/face-snapshot">Snapshot</a><a href="/edu/state">EDU State</a></div><div class="grid"><div class="card"><h2>1. 手機 MIC</h2><p class="hint">V14G 先啟用手機 MIC 權限，立即釋放 getUserMedia，再啟動原始 RT7 的 Chrome webkitSpeechRecognition。</p><button id="btnMic" class="mic" type="button" onclick="window.rt7V14StartMic&&window.rt7V14StartMic()">🎙️</button><button id="btnText" class="blue" type="button" onclick="window.rt7V14AskText&&window.rt7V14AskText()">文字詢問</button><button id="btnStop" class="red" type="button" onclick="window.rt7V14Stop&&window.rt7V14Stop()">停止播放</button><textarea id="question" rows="3">現在看到什麼？</textarea><div id="micStatus" class="hint">JS 載入中...</div></div><div class="card"><h2>2. 最新 ESP32 Snapshot</h2><img id="snap" src="/edu/face/latest.jpg?_=" onerror="this.style.display='none';document.getElementById('noSnap').style.display='block'"><p id="noSnap" class="bad" style="display:none">尚未收到 snapshot，請先讓 ESP32 上傳 FACE_GATE_PASS Candidate。</p><button id="btnSnap" type="button" onclick="window.rt7V14ReloadSnap&&window.rt7V14ReloadSnap()">重新載入 Snapshot</button></div></div><div class="card"><h2>3. AI 回答</h2><div id="answer" class="answer">尚未詢問</div><pre id="debug">BOOTING</pre></div><div class="card"><h2>4. 測試流程</h2><pre>1. 開頁後狀態應顯示 JS_READY V14G
2. 先按「文字詢問」確認 API 正常
3. 再按 🎙️，手機應跳出/使用麥克風權限
4. 說：現在看到什麼？
5. Speaker 播放回答</pre></div></div><script src="/edu/ai-voice-assistant-v14g.js?_=${Date.now()}"></script></body></html>`);
});
...
app.post('/api/v14/voice/camera-qa', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    ...
    const file = rt7V14GindLatestSnapshotFile_();
    ...
    const out = Object.assign({ ok: !!result.ok, version: VERSION, lesson:'RT7_EDU_AI_VOICE_ASSISTANT_V14G_MIC_UNLOCK_THEN_SPEECH_RECOGNITION', question, snapshot_bytes: buf.length, snapshot_meta: meta }, result);
