// RT7_EDU_AI_VOICE_ASSISTANT_V14_PHONE_MIC_SPEAKER_CAMERA_QA
...
// ============================================================================
// RT7 EDU LESSON 14 - AI Voice Assistant (Phone MIC + Phone SPEAKER + ESP32 Camera)
// Version: RT7_EDU_AI_VOICE_ASSISTANT_V14_PHONE_MIC_SPEAKER_CAMERA_QA
// Purpose:
//   手機 MIC：Browser SpeechRecognition
//   手機 SPEAKER：Browser speechSynthesis
//   ESP32 Camera：使用 Railway 最新 snapshot，問 OpenAI「現在看到什麼？」
// Does not modify ESP32 command / heartbeat / face match / liveness flow.
// ============================================================================
function rt7Lesson14AiVoicePage_() {
  return `<!doctype html>
<html lang="zh-Hant">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1">
<title>RT7 EDU Lesson 14 AI語音助理</title>
<style>
body{margin:0;background:#eef4f6;color:#10232e;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Noto Sans TC',Arial,sans-serif}
.wrap{max-width:960px;margin:0 auto;padding:16px}
.card{background:white;border-radius:18px;padding:16px;margin:14px 0;box-shadow:0 3px 12px #0001}
h1{font-size:26px;margin:12px 0}
button{font-size:18px;font-weight:900;border:0;border-radius:14px;padding:14px 16px;margin:6px;background:#0b9b5a;color:white}
button.blue{background:#0b6fa4} button.red{background:#c0392b} button.gray{background:#475569}
textarea,input{box-sizing:border-box;width:100%;font-size:17px;padding:12px;border:1px solid #ccd6dc;border-radius:12px;margin:6px 0}
pre{background:#f5f7f8;border-radius:12px;padding:12px;overflow:auto;white-space:pre-wrap;word-break:break-word}
img{width:100%;max-width:640px;border:1px solid #d8e1e7;border-radius:14px;background:#f8fafc}
.tag{display:inline-block;background:#e9f7ef;color:#087848;border-radius:999px;padding:5px 11px;font-size:13px;font-weight:900}
.ok{color:#079b50;font-weight:900}.bad{color:#d33;font-weight:900}.hint{color:#64748b;line-height:1.6}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px}
.bigmic{font-size:22px;padding:18px 20px;border-radius:999px;background:#ef4444}
</style>
</head>
<body>
<div class="wrap">
<h1>RT7 EDU Lesson 14：AI語音助理</h1>
<p><span class="tag">Phone MIC</span> <span class="tag">Phone SPEAKER</span> <span class="tag">ESP32 Camera Snapshot</span> <span class="tag">OpenAI Vision</span></p>

<div class="card">
<h2>1. 手機語音詢問 ESP32 鏡頭</h2>
<p class="hint">按下按鈕後，手機會開啟 MIC。請說：「現在看到什麼？」或「請問鏡頭現在看到什麼？」回答會從手機 Speaker 播放。</p>
<button class="bigmic" onclick="startMicAsk()">🎙️ 按住流程：開始說話</button>
<button class="blue" onclick="askDefault()">直接問：現在看到什麼？</button>
<button class="gray" onclick="stopSpeaking()">停止播放</button>
<p id="micStatus" class="hint">READY</p>
</div>

<div class="grid">
<div class="card">
<h2>2. 問題文字</h2>
<textarea id="question" rows="4">請問 ESP32 鏡頭現在看到什麼？請用繁體中文簡短回答，並說明是否有人、門口是否正常。</textarea>
<button onclick="askTyped()">送出文字問題</button>
</div>
<div class="card">
<h2>3. 最新 Snapshot</h2>
<img id="snap" src="/api/rt7/camera/latest.jpg?_=${Date.now()}" onerror="this.alt='尚未收到 ESP32 Snapshot';">
<p><button class="blue" onclick="refreshSnapshot()">重新整理照片</button></p>
</div>
</div>

<div class="card">
<h2>4. AI 回答</h2>
<pre id="answer">等待詢問...</pre>
</div>
...
<div class="card">
<h2>6. 測試步驟</h2>
<pre>1. ESP32 保持上線，並持續/手動上傳 snapshot 到 Railway。
2. 手機 Chrome 開啟本頁。
3. 按「開始說話」。
4. 說：「現在看到什麼？」
5. Railway 使用最新 ESP32 snapshot 呼叫 OpenAI Vision。
6. 手機 Speaker 播放回答。</pre>
</div>
</div>

<script>
function q(id){return document.getElementById(id)}
function log(x){q('debug').textContent = typeof x === 'string' ? x : JSON.stringify(x,null,2)}
function setAns(t){q('answer').textContent=String(t||'')}
function setMic(t){q('micStatus').innerHTML=String(t||'')}
function refreshSnapshot(){q('snap').src='/api/rt7/camera/latest.jpg?_='+Date.now()}
function stopSpeaking(){try{speechSynthesis.cancel()}catch(e){} setMic('已停止播放')}
...
function speak(txt){
  ...
  return new Promise(function(resolve){
    ...
    try{
      ...
      speechSynthesis.speak(u);
      setTimeout(resolve, Math.min(90000, Math.max(12000, txt.length*450)));
    }catch(e){resolve()}
  })
}

async function postJson(url,data){
  var r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data||{}),cache:'no-store'});
  var tx=await r.text();
  try{return JSON.parse(tx)}catch(e){return{ok:r.ok,status:r.status,raw:tx}}
}

async function askText(text){
  text=String(text||'').trim() || '請問 ESP32 鏡頭現在看到什麼？';
  q('question').value=text;
  setAns('AI 分析中：'+text);
  setMic('AI 分析中...');
  log({asking:text});
  try{
    var r=await postJson('/api/rt7/lesson14/voice_vision',{text:text,mode:'vision',lesson:'V14_PHONE_MIC_SPEAKER_CAMERA_QA'});
    log(r);
    var ans=r.answer || r.error || 'AI 無回應';
    setAns(ans);
    refreshSnapshot();
    await speak(ans);
    setMic(r.ok ? '<span class="ok">完成</span>' : '<span class="bad">失敗</span>');
  }catch(e){
    setAns('AI語音助理失敗：'+(e.message||e));
    setMic('<span class="bad">失敗</span>');
    log(String(e.stack||e));
  }
}

function askDefault(){askText('請問 ESP32 鏡頭現在看到什麼？請用繁體中文簡短回答。')}
function askTyped(){askText(q('question').value)}

function startMicAsk(){
  var SR=window.SpeechRecognition || window.webkitSpeechRecognition;
  if(!SR){
    var t=prompt('此瀏覽器不支援語音辨識，請輸入問題：','請問 ESP32 鏡頭現在看到什麼？')||'';
    if(t) askText(t);
    return;
  }
  try{
    ...
    rec.interimResults=false;
    rec.maxAlternatives=1;
    setMic('<span class="ok">請開始說話...</span>');
    rec.onresult=function(ev){
      var text='';
      try{text=ev.results[0][0].transcript||''}catch(e){}
      setMic('已收到：'+text);
      askText(text || '請問 ESP32 鏡頭現在看到什麼？');
    };
    rec.onerror=function(ev){
      setMic('<span class="bad">語音辨識失敗：'+(ev.error||'unknown')+'</span>');
      log(ev);
    };
    rec.onend=function(){ log('SpeechRecognition ended'); };
    rec.start();
  }catch(e){
    var t2=prompt('語音辨識無法啟動，請輸入問題：','請問 ESP32 鏡頭現在看到什麼？')||'';
    if(t2) askText(t2);
  }
}
setInterval(function(){ refreshSnapshot(); }, 10000);
</script>
</body>
</html>`;
}

app.get(['/edu/ai-voice-assistant','/edu/lesson14','/edu/lesson14-ai-voice'], (req,res)=>{
  res.type('html').send(rt7Lesson14AiVoicePage_());
});

app.get('/api/rt7/lesson14/state', (req,res)=>{
  res.json({
    ok:true,
    version:'RT7_EDU_AI_VOICE_ASSISTANT_V14_PHONE_MIC_SPEAKER_CAMERA_QA',
    openai_key:!!safeString(process.env.OPENAI_API_KEY).trim(),
    latest_snapshot: (typeof getSnapshotMeta_ === 'function') ? getSnapshotMeta_() : null,
    last_vision: cloudState && cloudState.last_vision || null,
    last_voice: cloudState && cloudState.last_voice || null
  });
});

app.post('/api/rt7/lesson14/voice_vision', async (req,res)=>{
  const started=Date.now();
  try{
    const text=safeString(req.body && req.body.text || '請問 ESP32 鏡頭現在看到什麼？').trim() || '請問 ESP32 鏡頭現在看到什麼？';
    const out=await analyzeLatestSnapshot(text);
    const result=Object.assign({
      version:'RT7_EDU_AI_VOICE_ASSISTANT_V14_PHONE_MIC_SPEAKER_CAMERA_QA',
      lesson:'Lesson 14',
      text,
      voice_ms:Date.now()-started,
      phone_mic:'browser_speech_recognition',
      phone_speaker:'browser_speech_synthesis',
      camera_source:'latest ESP32 snapshot on Railway'
    }, out);
    cloudState.last_voice=Object.assign({time:nowIso()}, result);
    res.json(result);
  }catch(e){
    res.status(200).json({
      ok:false,
      ...
      answer:'AI語音助理失敗。請確認 Railway 已設定 OPENAI_API_KEY，且 ESP32 已上傳最新 snapshot。'
    });
  }
});
