...
ensureFile('face_match_events.json', []);
...
async function sendFaceMatchPush(result) {
  let subs = getPushSubscriptions();
  if (!subs.length) {
    console.log('[EDU][V9A][FACE_PUSH] no subscriptions');
    return { sent: 0, failed: 0 };
  }
  const payload = JSON.stringify({
  ...
  for (const sub of subs) {
    try {
      ...
      failed++;
      const code = e && (e.statusCode || e.status);
      console.log('[EDU][V9A][FACE_PUSH_FAIL]', code || '', e && e.message ? e.message : e);
      if (code !== 404 && code !== 410) alive.push(sub);
    }
  }
  if (alive.length !== subs.length) savePushSubscriptions(alive);
  console.log('[EDU][V9A][FACE_PUSH_SENT]', 'sent=' + sent, 'failed=' + failed, result.match_id);
  return { sent, failed };
}

// 第四堂// 第四堂：門鈴事件。ESP32 按 GPIO38 後 POST 到這裡。
...
app.post('/edu/face/snapshot', express.raw({ type: ['image/jpeg', 'application/octet-stream'], limit: '3mb' }), (req, res) => {
  ...
  if (String(req.query.auto_match || '') === '1') {
    runFaceMatch(master_uid, 'AUTO_SNAPSHOT').then(result => {
      res.json({ ok: true, version: VERSION, snapshot: shot, count: shots.length, auto_match: true, result });
    }).catch(e => {
      console.log('[EDU][V9A][AUTO_MATCH_ERR]', e && e.message ? e.message : e);
      res.json({ ok: true, version: VERSION, snapshot: shot, count: shots.length, auto_match: false, error: String(e && e.message ? e.message : e) });
    });
    return;
  }
...
function latestSnapshotMeta(master_uid){
  const uid = normalizeUid(master_uid || '');
  const shots = readJson('face_snapshots.json', []);
  return shots.find(s => !uid || normalizeUid(s.master_uid) === uid) || shots[0] || null;
}
const FACE_MATCH_FRESH_MS = 30 * 1000;
...
app.post('/edu/face/register', (req,res)=>{
  ...
  const buf=latestImageBuffer();
  const latest = latestSnapshotMeta(master_uid);
  if(!master_uid) return res.status(400).json({ok:false,error:'missing master_uid'});
  if(!buf || !latest) return res.status(400).json({ok:false,error:'no latest snapshot. Upload snapshot first.'});
  const community=getMasterCommunity(master_uid);
  const rec={
    ...
    community_id:community?community.community_id:'',
    community_name:community?community.community_name:'',
    fingerprint:imageFingerprint(buf),
    bytes:buf.length,
    registered_snapshot_id: latest.snapshot_id || '',
    registered_snapshot_at: latest.created_at || '',
  ...
  res.json({ok:true,version:VERSION,face:{face_id:rec.face_id,person_name:rec.person_name,community_id:rec.community_id,community_name:rec.community_name,master_uid:rec.master_uid,bytes:rec.bytes,registered_snapshot_id:rec.registered_snapshot_id,registered_snapshot_at:rec.registered_snapshot_at,registered_at:rec.registered_at}});
});
async function runFaceMatch(master_uid, source = 'WEB'){
  master_uid = normalizeUid(master_uid);
  const buf = latestImageBuffer();
  const latest = latestSnapshotMeta(master_uid);
  if(!master_uid) throw new Error('missing master_uid');
  if(!buf || !latest) throw new Error('no latest snapshot');
  const latestAgeMs = Date.now() - Date.parse(latest.created_at || '');
  if(!Number.isFinite(latestAgeMs) || latestAgeMs > FACE_MATCH_FRESH_MS){
    const result = {match_id:'MATCH-'+Date.now().toString(36).toUpperCase(),master_uid,best_name:'',best_face_id:'',match_score:0,face_match:false,liveness:'UNKNOWN',allow_open:false,command_id:'',source,latest_snapshot_id:latest.snapshot_id||'',snapshot_age_ms:Number.isFinite(latestAgeMs)?latestAgeMs:-1,blocked_reason:'SNAPSHOT_NOT_FRESH__PLEASE_CAPTURE_AGAIN',created_at:nowIso(),lesson:VERSION};
    let results = readJson('face_match_results.json',[]); results.unshift(result); results = results.slice(0,50); writeJson('face_match_results.json',results);
  ...
  let best = null;
  for(const f of db){
    const score = fingerprintScore(fp,f.fingerprint);
    if(!best || score > best.score) best = {...f, score};
  }
  const sameRegisteredSnapshot = !!(best && best.registered_snapshot_id && latest.snapshot_id && best.registered_snapshot_id === latest.snapshot_id);
  const match = !!best && best.score >= 95 && !sameRegisteredSnapshot;
  const liveness = buf.length > 2500 ? 'REAL' : 'UNKNOWN';
  const allow_open = match && liveness === 'REAL';
  let command = null;
  if(allow_open){
    const community = getMasterCommunity(master_uid);
    const cmd = {command_id:'CMD-'+Date.now().toString(36).toUpperCase(),command:'OPEN_DOOR',status:'PENDING',community_id:community?community.community_id:'',community_name:community?community.community_name:'',master_uid,relay_pin:40,pulse_ms:800,source:'FACE_MATCH_REALTIME',created_at:nowIso(),lesson:VERSION};
    let commands = readJson('commands.json',[]); commands.unshift(cmd); commands = commands.slice(0,50); writeJson('commands.json',commands); command = cmd;
  }
  const result = {match_id:'MATCH-'+Date.now().toString(36).toUpperCase(),master_uid,best_name:best?best.person_name:'',best_face_id:best?best.face_id:'',match_score:best?best.score:0,face_match:match,liveness,allow_open,command_id:command?command.command_id:'',source,latest_snapshot_id:latest.snapshot_id||'',registered_snapshot_id:best?best.registered_snapshot_id||'':'',snapshot_age_ms:latestAgeMs,blocked_reason:sameRegisteredSnapshot?'MATCH_BLOCKED_SAME_AS_REGISTER_SNAPSHOT':'',created_at:nowIso(),lesson:VERSION};
  let results = readJson('face_match_results.json',[]); results.unshift(result); results = results.slice(0,50); writeJson('face_match_results.json',results);
  let events = readJson('face_match_events.json',[]); events.unshift({...result, event_id:'FACE-'+Date.now().toString(36).toUpperCase()}); events = events.slice(0,50); writeJson('face_match_events.json',events);
  if(allow_open) result.push = await sendFaceMatchPush(result);
  return { result, command };
}

app.post('/edu/face/match', async (req,res)=>{
  try {
    const out = await runFaceMatch((req.body||{}).master_uid, 'WEB_BUTTON');
    res.json({ok:true,version:VERSION,result:out.result,command:out.command});
  } catch(e) {
    res.status(400).json({ok:false,version:VERSION,error:String(e && e.message ? e.message : e)});
  }
});
app.get('/edu/face/db', (_req,res)=>res.json({ok:true,version:VERSION,faces:readJson('face_db.json',[]).map(f=>({face_id:f.face_id,person_name:f.person_name,community_id:f.community_id,community_name:f.community_name,master_uid:f.master_uid,bytes:f.bytes,registered_snapshot_id:f.registered_snapshot_id||'',registered_snapshot_at:f.registered_snapshot_at||'',registered_at:f.registered_at}))}));
...
h+='<div class="card"><h2>3. Face DB</h2><table><tr><th>Face ID</th><th>Name</th><th>Community</th><th>UID</th><th>Register Snapshot</th><th>Time</th></tr>'+((db.faces||[]).map(f=>'<tr><td>'+esc(f.face_id)+'</td><td><b>'+esc(f.person_name)+'</b></td><td>'+esc(f.community_name)+'</td><td>'+esc(f.master_uid)+'</td><td>'+esc(f.registered_snapshot_id||'')+'</td><td>'+esc(f.registered_at)+'</td></tr>').join('')||'<tr><td colspan="6" class="hint">尚未註冊</td></tr>')+'</table></div>';
h+='<div class="card"><h2>4. 辨識測試</h2><p class="hint">用最新 Snapshot 與 Face DB 比對；必須先重新拍照，不能拿註冊同一張照片直接比對。MATCH + LIVENESS=REAL 才自動送出 OPEN_DOOR。</p><select id="match_uid">'+opts(cs)+'</select><button onclick="matchFace()">做人臉辨識測試</button></div>';
h+='<div class="card"><h2>5. Match Results</h2><table><tr><th>Match ID</th><th>Name</th><th>Score</th><th>Liveness</th><th>Door</th><th>Command</th><th>Block Reason</th><th>Time</th></tr>'+((rr.results||[]).map(r=>'<tr><td>'+esc(r.match_id)+'</td><td>'+esc(r.best_name)+'</td><td>'+esc(r.match_score)+'%</td><td>'+esc(r.liveness)+'</td><td class="'+(r.allow_open?'ok':'bad')+'">'+(r.allow_open?'OPEN':'LOCK')+'</td><td>'+esc(r.command_id)+'</td><td>'+esc(r.blocked_reason||'')+'</td><td>'+esc(r.created_at)+'</td></tr>').join('')||'<tr><td colspan="9" class="hint">尚無辨識結果</td></tr>')+'</table></div>';
...
h+='<div class="card"><h2>4. Match Results</h2><table><tr><th>Match ID</th><th>Name</th><th>Score</th><th>Liveness</th><th>Door</th><th>Command</th><th>Source</th><th>Block Reason</th><th>Time</th></tr>'+((rr.results||[]).slice(0,12).map(r=>'<tr><td>'+esc(r.match_id)+'</td><td>'+esc(r.best_name)+'</td><td>'+esc(r.match_score)+'%</td><td>'+esc(r.liveness)+'</td><td class="'+(r.allow_open?'ok':'bad')+'">'+(r.allow_open?'OPEN':'LOCK')+'</td><td>'+esc(r.command_id)+'</td><td>'+esc(r.source||'')+'</td><td>'+esc(r.blocked_reason||'')+'</td><td>'+esc(r.created_at)+'</td></tr>').join('')||'<tr><td colspan="9" class="hint">尚無辨識結果</td></tr>')+'</table></div>';
...
function renderProductionFaceDoorbellPage(){return String.raw`<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RT7 EDU PRODUCTION FACE DOORBELL V10A</title><style>body{font-family:Arial,'Noto Sans TC',sans-serif;background:#eef4f6;margin:0;color:#10232e}.wrap{max-width:1120px;margin:20px auto;padding:16px}.card{background:#fff;border-radius:14px;padding:18px;margin:14px 0;box-shadow:0 2px 8px #0001}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px}button{font-size:16px;padding:10px;border-radius:8px;border:0;margin:4px;background:#0b9b5a;color:#fff}.blue{background:#0b6fa4}.danger{background:#c0392b}.tag{display:inline-block;background:#e9f7ef;color:#087848;border-radius:999px;padding:4px 10px;font-size:13px}.hint{color:#64748b;line-height:1.6}.ok{color:#079b50;font-weight:bold}.bad{color:#c0392b;font-weight:bold}table{width:100%;border-collapse:collapse}th,td{padding:8px;border-bottom:1px solid #e5edf1;text-align:left;word-break:break-all}img.snap{width:100%;max-width:640px;border-radius:12px;border:1px solid #d8e1e7;background:#f8fafc}pre{background:#f5f7f8;padding:10px;border-radius:8px;overflow:auto}.warn{background:#fff8e1;border-left:5px solid #f2c94c}.flow{font-size:18px;line-height:1.9}</style></head><body><div class="wrap"><h1>RT7 EDU PRODUCTION FACE DOORBELL V10A</h1><p><span class="tag">第十堂課</span> 教育版收斂到正式版：門鈴、推播、Snapshot、人臉辨識、活體檢測、開門控制。</p><p><button class="blue" onclick="location.href='/edu/face-preview'">手機預覽 / 手動拍照</button><button class="blue" onclick="location.href='/edu/face-recognition'">人臉註冊與辨識</button><button class="blue" onclick="location.href='/edu/push'">手機推播</button><button class="blue" onclick="location.href='/edu/open-door'">開門命令</button></p><div id="app">載入中...</div></div><script>
async function api(p,o){const r=await fetch(p,Object.assign({headers:{'Content-Type':'application/json'}},o||{}));let j={};try{j=await r.json()}catch(e){} if(!r.ok)j.http_status=r.status;return j} async function post(p,d){return api(p,{method:'POST',body:JSON.stringify(d||{})})} function esc(s){return String(s==null?'':s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
async function load(){const st=await api('/edu/state'), snaps=await api('/edu/face/snapshots'), db=await api('/edu/face/db'), rr=await api('/edu/face/results'), subs=await api('/edu/push/subscriptions'); const latest=(snaps.snapshots||[])[0]; let h='';
h+='<div class="card"><h2>1. V10 主流程</h2><div class="flow">ESP32 Camera / TFT 預覽 → 手機按「請 ESP32 拍照」 → Railway Snapshot → Face DB / Face Match → Liveness=REAL → Web Push → OPEN_DOOR → ESP32 GPIO40 Relay</div></div>';
h+='<div class="grid"><div class="card"><h2>Master</h2><p class="hint">設備數：'+Object.keys(st.masters||{}).length+'</p></div><div class="card"><h2>Community</h2><p class="hint">社區數：'+(st.communities||[]).length+'</p></div><div class="card"><h2>Push</h2><p class="hint">訂閱數：'+(subs.count||0)+'</p></div><div class="card"><h2>Face DB</h2><p class="hint">人臉數：'+((db.faces||[]).length)+'</p></div></div>';
h+='<div class="card"><h2>2. 最新 Snapshot</h2>'; if(latest){h+='<p class="hint">'+esc(latest.snapshot_id)+'｜'+esc(latest.community_name)+'｜bytes='+esc(latest.bytes)+'｜'+esc(latest.created_at)+'</p><img class="snap" src="/edu/face/latest.jpg?ts='+Date.now()+'">'} else h+='<p class="bad">尚未收到 Snapshot，請先到「手機預覽 / 手動拍照」。</p>'; h+='</div>';
h+='<div class="card"><h2>3. Match Results</h2><table><tr><th>Match ID</th><th>Name</th><th>Score</th><th>Liveness</th><th>Door</th><th>Command</th><th>Source</th><th>Block Reason</th><th>Time</th></tr>'+((rr.results||[]).slice(0,10).map(r=>'<tr><td>'+esc(r.match_id)+'</td><td>'+esc(r.best_name)+'</td><td>'+esc(r.match_score)+'%</td><td>'+esc(r.liveness)+'</td><td class="'+(r.allow_open?'ok':'bad')+'">'+(r.allow_open?'OPEN':'LOCK')+'</td><td>'+esc(r.command_id)+'</td><td>'+esc(r.source||'MANUAL')+'</td><td>'+esc(r.blocked_reason||'')+'</td><td>'+esc(r.created_at)+'</td></tr>').join('')||'<tr><td colspan="9" class="hint">尚無辨識結果</td></tr>')+'</table></div>';
h+='<div class="card"><h2>4. OPEN_DOOR Commands</h2><table><tr><th>Command ID</th><th>Command</th><th>Status</th><th>Source</th><th>Delivered</th><th>ACK</th></tr>'+((st.commands||[]).slice(0,10).map(c=>'<tr><td>'+esc(c.command_id)+'</td><td>'+esc(c.command)+'</td><td class="'+(c.status==='DONE'?'ok':'bad')+'">'+esc(c.status)+'</td><td>'+esc(c.source)+'</td><td>'+esc(c.delivered_at||'')+'</td><td>'+esc(c.ack_at||'')+'</td></tr>').join('')||'<tr><td colspan="6" class="hint">尚無命令</td></tr>')+'</table></div>';
h+='<div class="card warn"><h2>5. 教育版對照正式版</h2><pre>教育版 V10：Fingerprint Face Match / Liveness 教學判斷\n正式版 V5_8E2：OpenAI Face Match + OpenAI Liveness + Security Guard\n\n教育版用來教流程；正式版用於社區門禁實戰。</pre></div>';
document.getElementById('app').innerHTML=h} load(); setInterval(load,10000);
</script></body></html>`}
...
app.listen(PORT, () => console.log('[' + VERSION + '] http://localhost:' + PORT + '/edu/production-face-doorbell'));
