...
const SERVER_VERSION = 'RT7_CH7B2_FACE_RECOGNITION_SECURITY_GUARD';
...
// ======================================================
// RT7_CH7B2_FACE_RECOGNITION_SECURITY_GUARD
// 回歸原專案主軸：門禁人臉辨識 + 活體檢測 + 開門 + 紀錄 + 推播
// ESP32 Snapshot -> Railway Face DB -> Railway Face Match -> OpenAI Liveness -> OPEN_DOOR
// Page:
//   /rt7_ch7_face_security_guard
// APIs:
//   GET  /api/ch7/face-security/state
//   GET  /api/ch7/face-security/log
//   POST /api/ch7/face-security/snapshot
//   POST /api/ch7/face-security/match_latest
//   GET  /api/rt7/device/command        // compatibility alias for ESP32 polling
// ======================================================
const CH7_FACE_SECURITY_LOG_FILE = path.join(DATA_DIR, 'ch7_face_security_log.json');
function ch7FaceSecEnsure_(){ ensureDataDir(); if(!fs.existsSync(CH7_FACE_SECURITY_LOG_FILE)) fs.writeFileSync(CH7_FACE_SECURITY_LOG_FILE, '[]', 'utf8'); }
function ch7FaceSecReadLog_(){ ch7FaceSecEnsure_(); try{ const a=JSON.parse(fs.readFileSync(CH7_FACE_SECURITY_LOG_FILE,'utf8')||'[]'); return Array.isArray(a)?a:[]; }catch(_){ return []; } }
function ch7FaceSecSaveLog_(arr){ ch7FaceSecEnsure_(); fs.writeFileSync(CH7_FACE_SECURITY_LOG_FILE, JSON.stringify(Array.isArray(arr)?arr.slice(0,500):[], null, 2), 'utf8'); }
function ch7FaceSecAppend_(ev){ const a=ch7FaceSecReadLog_(); a.unshift(Object.assign({time:nowIso()}, ev||{})); ch7FaceSecSaveLog_(a); return a[0]; }
function ch7FaceSecCleanB64_(image){
  let s=safeString(image||'').trim();
  const m=s.match(/^data:image\/[a-zA-Z0-9.+-]+;base64,(.*)$/);
  if(m) s=m[1];
  return s.replace(/\s+/g,'');
}
function ch7FaceSecHash_(b64){
  try{ return crypto.createHash('sha1').update(String(b64||'').slice(0,200000)).digest('hex').slice(0,12); }catch(_){ return String(Date.now()); }
}
function ch7FaceSecLatestFromB64_(b64, source){
  const clean=ch7FaceSecCleanB64_(b64);
  const buf=Buffer.from(clean,'base64');
  const hash=ch7FaceSecHash_(clean);
  try{ fs.writeFileSync(path.join(DATA_DIR, 'ch7_face_security_last_snapshot.jpg'), buf); }catch(_){ }
  return { b64:clean, bytes:buf.length, snap_time:nowIso(), snap_source:source||'ch7_face_security_upload', snap_hash:hash, snap_age_ms:0, snap_wait_ms:0, snap_forced_realtime:true, snap_request_ws_sent:0, snap_live_frame_fallback:false };
}
function ch7FaceSecQueueOpen_(result, reqBody){
  const deviceId = normalizeDoorCommandDeviceId_(safeString(reqBody && (reqBody.device_id || reqBody.device) || '#1'));
  const pulse = Number((reqBody && reqBody.pulse_ms) || process.env.RT7_RELAY_PULSE_MS || 800);
  const pin = Number((reqBody && reqBody.pin) || process.env.RT7_RELAY_PIN || 40);
  if (Array.isArray(pendingCommands)) pendingCommands = pendingCommands.filter(c => !commandMatchesDevice_(c, deviceId));
  const cmd = queueCommand({
    command:'door_open', action:'door_open', cmd:'OPEN_DOOR', type:'door_open',
    source:'face_recognition_security_guard',
    device_id:deviceId, requested_device_id:safeString(reqBody && reqBody.device_id || '#1') || '#1',
    master_uid:safeString(reqBody && reqBody.master_uid || ''), community_id:safeString(reqBody && reqBody.community_id || ''), community_name:safeString(reqBody && reqBody.community_name || ''),
    pin, pulse_ms:pulse,
    matched_name:result && result.matched_name || '', confidence:Number(result && result.confidence || 0),
    liveness:result && result.liveness_label || '', liveness_confidence:result && result.liveness && result.liveness.confidence || 0,
    message:'Face Recognition Security Guard: MATCH + LIVENESS 通過，自動開門'
  });
  const wsPayload = { ok:true, type:'door_open', command:'door_open', action:'door_open', cmd:'OPEN_DOOR', id:cmd.id, command_id:cmd.id, device_id:deviceId, pin, pulse_ms:pulse, source:'face_recognition_security_guard_ws', time:nowIso() };
  const wsSent = rt7SendWsJsonToEsp_(wsPayload);
  cmd.ws_sent = wsSent;
  cloudState.last_face_security_open = cmd;
  return { command:cmd, ws_sent:wsSent };
}
async function ch7FaceSecPush_(result, decision, reqBody){
  try{
    const title = decision === 'OPEN_DOOR' ? '✅ RT7 人臉辨識開門' : '⛔ RT7 人臉辨識拒絕';
    const name = result && result.matched_name ? result.matched_name : 'unknown';
    const body = decision === 'OPEN_DOOR'
      ? ('MATCH ' + name + ' / ' + (result.confidence||0) + '% / LIVENESS=' + (result.liveness_label||''))
      : ('DOOR=DENY / ' + (result && (result.reason || result.summary) || 'FACE_MATCH_FAIL'));
    return await rt7SendPushDoorbell_({ type:'face_security_guard', title, body, url:'/rt7_ch7_face_security_guard', tag:'rt7-face-security', face_security:true, decision, community_id:reqBody && reqBody.community_id || '', master_uid:reqBody && reqBody.master_uid || '' });
  }catch(e){ return {ok:false, sent:0, error:String(e && e.message || e)}; }
}
async function ch7FaceSecRun_(latest, reqBody){
  const started=Date.now();
  let match=null, open=null, push=null;
  try{
    match = latest ? await rt7FaceMatchLatestCore_(latest, { ch7_face_security_guard:true }) : await rt7FaceMatchLatest_();
    const allow = !!(match && (match.door_allow || match.known_face) && match.liveness_pass !== false);
    const decision = allow ? 'OPEN_DOOR' : 'DENY';
    if (allow) open = ch7FaceSecQueueOpen_(match, reqBody||{});
    push = await ch7FaceSecPush_(match, decision, reqBody||{});
    const ev = ch7FaceSecAppend_({
      ok:true, kind:'face_security_guard', decision, door_open:allow, source:reqBody && reqBody.source || (latest ? 'snapshot_upload' : 'latest_snapshot'),
      community_id:reqBody && reqBody.community_id || '', community_name:reqBody && reqBody.community_name || '', master_uid:reqBody && reqBody.master_uid || '',
      matched_name:match && match.matched_name || '', confidence:match && match.confidence || 0,
      face_match_pass:!!(match && match.face_match_pass), liveness_pass:!!(match && match.liveness_pass), liveness_label:match && match.liveness_label || '',
      face_found:!!(match && match.face_found), face_count:match && match.face_count || 0, reason:match && match.reason || '', summary:match && match.summary || '',
      command:open && open.command || null, ws_sent:open && open.ws_sent || 0, push_status:push && (push.status || (push.sent?'SENT':'NO_SUBSCRIBER')) || '', push_sent:push && push.sent || 0,
      elapsed_ms:Date.now()-started, match
    });
    cloudState.last_face_security_guard = ev;
    broadcast('face_security_guard', ev);
    return { ok:true, version:'RT7_CH7B2_FACE_RECOGNITION_SECURITY_GUARD', event:ev, decision, match, open, push };
  }catch(e){
    const ev = ch7FaceSecAppend_({ ok:false, kind:'face_security_guard', decision:'ERROR', error:String(e && e.message || e), elapsed_ms:Date.now()-started });
    return { ok:false, version:'RT7_CH7B2_FACE_RECOGNITION_SECURITY_GUARD', event:ev, error:String(e && e.message || e) };
  }
}
app.get('/api/ch7/face-security/state', (req,res)=>{
  res.json({ ok:true, version:'RT7_CH7B2_FACE_RECOGNITION_SECURITY_GUARD', faces:rt7ReadFaces_().length, last_face_match:cloudState.last_face_match||null, last_face_security_guard:cloudState.last_face_security_guard||null, logs:ch7FaceSecReadLog_().slice(0,50), push_subscriptions:rt7ReadPushSubs_().length, latest_snapshot:getSnapshotMeta_(), commands_pending:Array.isArray(pendingCommands)?pendingCommands.length:0 });
});
app.get('/api/ch7/face-security/log', (req,res)=>res.json({ ok:true, logs:ch7FaceSecReadLog_().slice(0,200) }));
app.post('/api/ch7/face-security/snapshot', async (req,res)=>{
  const image=safeString(req.body && (req.body.image || req.body.image_b64 || req.body.jpeg_b64) || '');
  if(!image) return res.status(400).json({ ok:false, error:'MISSING_IMAGE', hint:'POST JSON { image:"data:image/jpeg;base64,..." }' });
  const latest=ch7FaceSecLatestFromB64_(image, safeString(req.body && req.body.source || 'esp32_ch7_face_security'));
  const out=await ch7FaceSecRun_(latest, req.body||{});
  res.status(out.ok?200:500).json(out);
});
app.post('/api/ch7/face-security/match_latest', async (req,res)=>{
  const out=await ch7FaceSecRun_(null, req.body||{});
  res.status(out.ok?200:500).json(out);
});
app.get('/api/rt7/device/command', (req,res)=>{
  const id = normalizeDoorCommandDeviceId_(req.query.device_id || req.query.device || '#1');
  const matches = Array.isArray(pendingCommands) ? pendingCommands.filter(c => commandMatchesDevice_(c, id)) : [];
  const cmd = matches.find(c => c && (c.cmd === 'OPEN_DOOR' || c.command === 'door_open' || c.action === 'door_open')) || matches[0] || null;
  res.json({ ok:true, version:'RT7_CH7B2_FACE_RECOGNITION_SECURITY_GUARD', command:cmd ? Object.assign({ cmd:cmd.cmd || (cmd.command==='door_open'?'OPEN_DOOR':cmd.command) }, cmd) : null, has_command:!!cmd, pending:Array.isArray(pendingCommands)?pendingCommands.length:0, device_id:id });
});
app.get('/api/rt7/device/command/ack', ackCommand);
app.post('/api/rt7/device/command/ack', ackCommand);
app.get('/rt7_ch7_face_security_guard', (req,res)=>res.type('html').send(`<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RT7 CH7B2 Face Security Guard</title><style>
body{font-family:Arial,'Noto Sans TC',sans-serif;background:#eef4f6;margin:0;color:#10232e}.wrap{max-width:1050px;margin:auto;padding:16px}.card{background:#fff;border-radius:16px;padding:18px;margin:14px 0;box-shadow:0 2px 8px #0001}button,input{font-size:16px;padding:10px;margin:4px;border-radius:8px;border:1px solid #ccd6dc}button{background:#0b78d0;color:#fff;border:0}.green{background:#0b9b5a}.red{background:#c0392b}.gray{background:#64748b}.pill{display:inline-block;padding:3px 9px;border-radius:999px;background:#e9f7ef;color:#0b7a43;font-weight:bold}pre{background:#f5f7f8;padding:10px;border-radius:8px;overflow:auto;white-space:pre-wrap}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}@media(max-width:760px){.grid{grid-template-columns:1fr}}
</style></head><body><div class="wrap"><h1>RT7 CH7B2 Face Recognition Security Guard</h1><p>門禁人臉辨識：Face DB + Railway Face Match + OpenAI Liveness，通過才送 OPEN_DOOR。</p><div id="app">載入中...</div></div><script>
...
async function render(){const st=await api('/api/ch7/face-security/state');let h='';h+='<div class="card"><h2>1. 狀態</h2><p>Face DB：<span class="pill">'+esc(st.faces)+'</span> Push：<span class="pill">'+esc(st.push_subscriptions)+'</span> Pending Command：<span class="pill">'+esc(st.commands_pending)+'</span></p></div>';h+='<div class="card"><h2>2. 手機/電腦 Snapshot 測試</h2><input id="photo" type="file" accept="image/*" capture="environment"><input id="community" placeholder="community_id，可空白"><input id="master" placeholder="master_uid，可空白"><button class="green" onclick="uploadTest()">上傳 Snapshot + Face Security Guard</button><button onclick="matchLatest()">使用 ESP32 最新 Snapshot 比對</button><button class="gray" onclick="render()">重新整理</button></div>';h+='<div class="card"><h2>3. 最近紀錄</h2><pre>'+esc(JSON.stringify((st.logs||[]).slice(0,20),null,2))+'</pre></div>';h+='<div class="card"><h2>4. 最新結果</h2><pre id="out">READY</pre></div>';app.innerHTML=h;}
async function uploadTest(){try{const image=await fileDataUrl(photo);const r=await post('/api/ch7/face-security/snapshot',{image,community_id:community.value,master_uid:master.value,source:'web_face_security_guard'});out(r);setTimeout(render,1200)}catch(e){out({ok:false,error:String(e)})}}
async function matchLatest(){const r=await post('/api/ch7/face-security/match_latest',{community_id:community.value,master_uid:master.value,source:'latest_snapshot'});out(r);setTimeout(render,1200)}
render();
</script></body></html>`));
// ======================================================
// End RT7_CH7B2_FACE_RECOGNITION_SECURITY_GUARD
// ======================================================
