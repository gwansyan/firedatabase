...
</style></head><body><div class="wrap"><h1>第14堂 AI語音助理 V14F</h1><p class="hint">維持原始 RT7：手機 MIC → Chrome webkitSpeechRecognition → 文字 → Railway Snapshot → OpenAI Vision → 手機 Speaker。</p><div class="top"><a href="/edu/community/register">社區註冊</a><a href="/edu/two-step-liveness">V12B 二步活體</a><a href="/edu/face-snapshot">Snapshot</a><a href="/edu/state">EDU State</a></div><div class="grid"><div class="card"><h2>1. 手機 MIC</h2><p class="hint">V14F 先啟用手機 MIC 權限，再啟動原始 RT7 的 Chrome webkitSpeechRecognition。</p><button id="btnMic" class="mic" type="button" onclick="window.rt7V14StartMic&&window.rt7V14StartMic()">🎙️</button><button id="btnText" class="blue" type="button" onclick="window.rt7V14AskText&&window.rt7V14AskText()">文字詢問</button><button id="btnStop" class="red" type="button" onclick="window.rt7V14Stop&&window.rt7V14Stop()">停止播放</button><textarea id="question" rows="3">現在看到什麼？</textarea><div id="micStatus" class="hint">JS 載入中...</div></div><div class="card"><h2>2. 最新 ESP32 Snapshot</h2><img id="snap" src="/edu/face/latest.jpg?_=" onerror="this.style.display='none';document.getElementById('noSnap').style.display='block'"><p id="noSnap" class="bad" style="display:none">尚未收到 snapshot，請先讓 ESP32 上傳 FACE_GATE_PASS Candidate。</p><button id="btnSnap" type="button" onclick="window.rt7V14ReloadSnap&&window.rt7V14ReloadSnap()">重新載入 Snapshot</button></div></div><div class="card"><h2>3. AI 回答</h2><div id="answer" class="answer">尚未詢問</div><pre id="debug">BOOTING</pre></div><div class="card"><h2>4. 測試流程</h2><pre>1. 開頁後狀態應顯示 JS_READY V14F
2. 先按「文字詢問」確認 API 正常
3. 再按 🎙️，手機應跳出/使用麥克風權限
4. 說：現在看到什麼？
5. Speaker 播放回答</pre></div></div><script src="/edu/ai-voice-assistant-v14f.js?_=${Date.now()}"></script></body></html>`);
});
